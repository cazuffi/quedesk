...
export function StatusBar({ dbReady, dbError }: StatusBarProps) {
  ...
  return (
    <footer className="hidden shrink-0 items-center justify-between border-t-2 border-[var(--color-border)] bg-[var(--color-surface-raised)] px-4 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] text-xs text-[var(--color-text-muted)] sm:flex sm:px-5 sm:py-1.5 sm:text-[10px] sm:pb-[max(0.375rem,env(safe-area-inset-bottom))]">
